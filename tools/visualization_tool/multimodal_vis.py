...
def _controls(biopac_duration, default_biopac_offset, mo):
    ...
    canonical_envelope_ms = mo.ui.number(
        start=50.0, stop=1000.0, step=10.0, value=250.0, label="Envelope window (ms)"
    )
...
def _canonical_pipeline_plot(
    canonical_plot, go, make_subplots, normalize, robust_range
):
    ...
    _fig.update_xaxes(title_text="Biopac time (s)", row=4, col=1)
