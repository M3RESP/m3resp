...
`m3resp` does not reimplement `eitprocessing` algorithms - it calls them
(see [../developer/architecture.md](../developer/architecture.md)'s
...
Migrating existing code means replacing manual calls into `eitprocessing`
...
See [../tutorials/eit-only.md](../tutorials/eit-only.md) for a full
end-to-end example, and [../migration.md](../migration.md) for cross-modality
bookkeeping that applies to EIT the same way it does to EMG.

## What does not change

`eitprocessing` remains independent, installable, and usable on its own -
`m3resp` depends on it, never the reverse. Passing a custom
`preprocess=callable`/`detector=callable` to the relevant `M3Session`/adapter
method still works, for anything not covered by the built-in conversions.
