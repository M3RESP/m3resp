...
- [`examples/annemijn_multimodal/annemijn.pipeline.yaml`](https://github.com/M3RESP/m3resp/blob/main/examples/annemijn_multimodal/annemijn.pipeline.yaml) — real EIT + diaphragm sEMG + airway-pressure recording; ECG-contaminated surface EMG cleaned with `emg.ecg_gating` before breath detection. No ventilator in this recording, so no Pocc steps; uses a dataset-specific `sync.estimate_offset` method rather than `session.sync_raw`.
