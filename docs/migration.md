...
|---|---|
| `eitprocessing.datahandling.loading.load_eit_data(path, vendor=...)` | `m3resp.load_eit(path, vendor=...)` or `session.load_eit(path, vendor=...)` |
...
|---|---|
| `resurfemg.data_connector.converter_functions.load_file(path)` | `m3resp.load_emg(path)` or `session.load_emg(path)` |
...
| Matching EIT/EMG/ventilator breaths by eyeballing timestamps | `m3resp.link_breaths_by_time(...)` or `session.link_breaths(time_tolerance=...)` -> `LinkedBreath` objects |
